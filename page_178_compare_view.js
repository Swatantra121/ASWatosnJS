...
function setUpMouseHander(p_element, p_mouseDragFunc, p_mouseDownFunc, p_mouseUpFunc, p_canvas, p_pog_index) {
    ...
    // p_canvas.parentElement.addEventListener("mousedown", doMouseDown);
    // p_canvas.parentElement.addEventListener("mousemove", doMouseMove);
    // p_canvas.parentElement.addEventListener("mouseup", doMouseUpHandler);
    ...
    // p_canvas.addEventListener("selectstart", function(e) { e.preventDefault(); });
}
...
function doMouseMove(p_x, p_y, p_event, p_prevX, p_prevY, p_canvas, p_pog_json_opp, p_jselector, p_pog_index, p_camera) {
     ...
     */
	try {
		// prevent default browser drag behavior during POG interaction
		if (g_dragging) {
			p_event.preventDefault();
			p_event.stopPropagation();
		}
...
function doMouseDown(p_x, p_y, p_startX, p_startY, p_event, p_canvas, p_context_call, p_pog_index) {
    ...
    try {
        ...
        // always track where the user started the mouse down for area selection
        g_startMouse.x = p_event.clientX + scroll_left - 20;
        g_startMouse.y = p_event.clientY + scroll_top;
...
// function reset_zoom(p_pog_index = "0") {
//     logDebug("function : reset_zoom", "S");
//     try {
//         debugger
//         if ((typeof g_pog_json !== "undefined" && g_pog_json.length > 0) || (typeof g_pog_json_comp !== "undefined" && g_pog_json_comp.length > 0)) {
